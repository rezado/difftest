...
bool Perfprocess::get_simulation_stats(long int dse_epoch) {
  ...
  uint64_t issue_fire_cnt = find_perfCnt("issue_fire_cnt");
  ...
  // Aggregated instruction counters (replaced individual fuType counters)
  uint64_t int_instr_cnts = find_perfCnt("int_instr_cnts");
  uint64_t fp_instr_cnts = find_perfCnt("fp_instr_cnts");
  uint64_t load_instr_cnts = find_perfCnt("load_instr_cnts");
  uint64_t store_instr_cnts = find_perfCnt("store_instr_cnts");
  ...
  uint64_t functionCalls = find_perfCnt("functionCalls");
  ...
  uint64_t dcache_conflict = find_perfCnt("dcache_conflict");
  uint64_t access_dtlb = find_perfCnt("access_dtlb");
  uint64_t miss_dtlb = find_perfCnt("miss_dtlb");
  uint64_t replace_dtlb = find_perfCnt("replace_dtlb");
  // Note: L2 counters use capital L
  ...
  uint64_t l2_conflict = find_perfCnt("L2_conflict");
  // ICache and BTB counters
  ...
  stats << "dse_epoch: " << dse_epoch << std::endl
        ...
        << "total_instructions: " << issue_fire_cnt << std::endl
        << "int_instructions: " << int_instr_cnts << std::endl
        << "fp_instructions: " << fp_instr_cnts << std::endl
        << "branch_instructions: " << commitInstrBranch << std::endl
        << "load_instructions: " << load_instr_cnts << std::endl
        << "store_instructions: " << store_instr_cnts << std::endl
        << "committed_instructions: " << commitUop << std::endl
        << "committed_int_instructions: " << int_instr_cnts << std::endl
        << "committed_fp_instructions: " << fp_instr_cnts << std::endl
        ...
        << "functionCalls: " << functionCalls << std::endl
        ...
        << "dcache_conflit: " << dcache_conflict << std::endl
        ...
        << "l2_conflit: " << l2_conflict << std::endl
